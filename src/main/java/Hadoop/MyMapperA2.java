 ...
 */
public class MyMapperA2 extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        String userid = linepar[0];
        String rating = linepar[2];
        String useridNrating = userid + "-" + rating;
        Integer movieid = Integer.parseInt(movieidS);
        String moviename = hm.get(movieid);
        context.write(new Text(moviename), new Text(useridNrating));
    }
}
